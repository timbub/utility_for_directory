...
void generate_manifest(char* abs_path, char* rel_path)
{
    ...
    char new_abs_path[MAX_LEN_NAME] = {0};           //absolute path for search
    char new_rel_path[MAX_LEN_NAME] = {0};           //relative path for output
    ...
    while((entry = readdir(dir)) != NULL)
    {
        ...
        snprintf(new_abs_path, sizeof(new_abs_path), "%s/%s", abs_path, entry->d_name); //create new absolute path for file

        if(lstat(new_abs_path, &buf) == -1)  //get info about file/directory
        {
        ...
        {
            //create new relative path for file
            if (rel_path[0] == '\0')
            {
            ...
            unsigned long hash = calculating_hash(new_abs_path);
            printf("%s : %lX\n", new_rel_path, hash);
        }
    }
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        if (lstat(new_abs_path, &buf) == -1)
        {
            fprintf(stderr, "ERROR: info about %s not load\n", entry->d_name);
            continue;
        }

        if (S_ISDIR(buf.st_mode)) //get info about file/directory
        {
            //create new relative path for directory
            if (rel_path[0] == '\0')
            {
                snprintf(new_rel_path, sizeof(new_rel_path), "%s", entry->d_name);
            } else
            ...
            generate_manifest(new_abs_path, new_rel_path);
        }
    }
    closedir(dir);
}
